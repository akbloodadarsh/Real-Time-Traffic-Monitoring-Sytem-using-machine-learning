...
def drawPred(classId, conf, left, top, right, bottom):
    ...
    if classId == 1 or classId == 2 or classId == 3 or classId == 5 or classId == 7:
        ...
        cv.rectangle(frame, (left, top - round(1.5 * labelSize[0])), (left + round(1.5 * labelSize[0]), top + baseLine)
        ,cv.FILLED)
...
are using there are multiple output layers giving out predictions. get_output_layers() function gives the names of the 
output layers. An output layer is not connected to any next layer
'''
...
while cv.waitKey(1) < 0:
    ...
    # Create a 4D blob from a frame cv.dnn.blobFromImage(image,scalefactor,size,mean, swapRB,crop,depth)
