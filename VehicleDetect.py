    ...
    if choice == '1':
        ...
        while True:
            ...
            if ret:
                ...
                # Set the i/p blob
                net.setInput(blob)
                # outs:-Array contains all info about objects detected,their position,confidence about the detection
                # forward:- (blob already set)compute o/p of all the layers and returns the blob
                outs = net.forward(getOutputsNames(net))
                # send the frame, detected obj,previous count and return the updated previous count
                prev_veh_count, prev_ped_count, p_frame, v_frame = postprocess(frame, p_frame, v_frame, outs,
...
if __name__ == '__main__':
    ...
    Label(mainwindow, text="Slide to save", bg="black", fg="white").pack(side=TOP, fill=X, padx=2, pady=2)
    ch = Scale(mainwindow, from_=0, to=1, orient=HORIZONTAL)
    ch.pack(side=TOP, fill=X, padx=2, pady=2)
    Label(mainwindow, text="Slide to use camera", bg="black", fg="white").pack(side=TOP, fill=X, padx=2, pady=2)
    feed = Scale(mainwindow, from_=0, to=1, orient=HORIZONTAL)
    feed.pack(side=TOP, fill=X, padx=2, pady=2)
